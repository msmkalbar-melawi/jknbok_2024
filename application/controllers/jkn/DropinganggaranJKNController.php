...
class DropinganggaranJKNController extends CI_Controller
{
    ...
    {
        $data['page_title'] = 'DROPING ANGGARAN JKN';
        $this->template->set('title', 'DROPING ANGGARAN JKN');
        $this->template->load('template', 'jkn/droppinganggaran/index', $data);
    }
    ...
    public function rekening_belanja()
    {
        ...
        // $kd_sub_kegiatan = '1.02.02.2.02.33';
        $anggaran = $this->db->query("SELECT a.kd_skpd as kd_skpd,a.nm_skpd as nm_skpd , b.jns_ang as jns_ang FROM ms_skpd a LEFT JOIN trhrka b
        ON a.kd_skpd=b.kd_skpd WHERE a.kd_skpd = '$kd_skpd' and 
        b.tgl_dpa in(SELECT MAX(tgl_dpa) from trhrka where kd_skpd=a.kd_skpd AND status='1')");
        $data = $anggaran->row();
        $jns_ang = $data->jns_ang;
        $result = array();
        $sql = $this->db->query("SELECT b.kd_sub_kegiatan, b.nm_sub_kegiatan, b.kd_rek6, b.nm_rek6 FROM trskpd a INNER JOIN trdrka b ON b.kd_skpd=a.kd_skpd AND b.jns_ang=a.jns_ang AND a.kd_sub_kegiatan=b.kd_sub_kegiatan WHERE a.jns_ang='$jns_ang' AND a.kd_skpd='$kd_skpd' AND a.kd_sub_kegiatan='$kd_sub_kegiatan'");
        foreach ($sql->result_array() as $resulte) {
            $result[] = array(
                'kd_sub_kegiatan' => $resulte['kd_sub_kegiatan'],
                'nm_sub_kegiatan' => $resulte['nm_sub_kegiatan'],
    ...
    //     $sql = "SELECT TOP $rows a.no_kas, a.tgl_kas, a.no_bukti,a.tgl_bukti,a.kd_skpd, (SELECT nm_skpd FROM ms_skpd_jkn WHERE kd_skpd=a.kd_skpd)as nm_skpd, a.Keterangan as keterangan, a.jenis, SUM(b.nilai) as nilai, a.no_transaksi FROM jkn_trhrka a INNER JOIN jkn_trdrka b ON b.kd_skpd=a.kd_skpd AND a.no_bukti=b.no_bukti AND b.jenis=a.jenis WHERE a.kd_skpd='$kd_skpd' AND a.jenis IN('1') AND a.no_bukti NOT IN (SELECT TOP $offset a.no_bukti FROM jkn_trhrka a INNER JOIN jkn_trdrka b ON b.kd_skpd=a.kd_skpd AND a.no_bukti=b.no_bukti WHERE a.kd_skpd='$kd_skpd' AND a.jenis IN('1') GROUP BY a.no_kas, a.tgl_kas, a.no_bukti,a.tgl_bukti,a.kd_skpd, a.Keterangan,a.jenis) GROUP BY a.no_kas, a.tgl_kas, a.no_bukti,a.tgl_bukti,a.kd_skpd, a.Keterangan,a.jenis,a.no_transaksi
    ...
    public function loaddata()
    {
        ...
        $sql = "SELECT kd_skpd, nm_skpd, kd_sub_kegiatan, nm_sub_kegiatan, kd_rek6, nm_rek6, nilai FROM jkn_trdrka WHERE kd_skpd='$kd_skpd' AND kd_sub_kegiatan='1.02.02.2.02.0026' GROUP BY kd_skpd, kd_sub_kegiatan,nm_skpd, kd_rek6, nm_rek6, nilai, nm_sub_kegiatan";
        $query1 = $this->db->query($sql);
        $result = array();
        foreach ($query1->result_array() as $resulte) {
            $row[] = array(
    ...
    public function loadingdata()
    {
        ...
        // Query
        $sql = "SELECT a.no_bukti, a.kd_sub_kegiatan, a.nm_sub_kegiatan, a.kd_rek6, a.nm_rek6, a.nilai FROM jkn_trdrka a INNER JOIN jkn_trhrka b ON b.no_bukti=a.no_bukti AND b.kd_skpd=a.kd_skpd AND a.jenis=b.jenis WHERE a.no_bukti='$no_bukti' AND b.kd_skpd='$kd_skpd' AND b.jenis IN('$jenis') AND b.no_transaksi='$no_transaksi'";
        $query1 = $this->db->query($sql);
        $result = array();
        $ii = 0;
    ...
    public function update_data()
    {
        ...
        if ($jumlah12 == 0) {
            if (!empty($lcnilai)) {
                $this->db->query("DELETE a from jkn_trdrka a INNER JOIN jkn_trhrka b ON b.kd_skpd=a.kd_skpd AND b.no_bukti=a.no_bukti AND a.jenis=b.jenis WHERE a.kd_skpd='$cskpd' AND a.no_bukti='$cno' AND a.jenis='$cjenis' AND b.no_transaksi='$no_trans_hidden'");
                $sql = $this->db->query("INSERT INTO $tabel $lckolom values $lcnilai");
                $this->db->query("UPDATE jkn_trhrka set tgl_kas='$ctgl', tgl_bukti='$ctgl', no_kas='$cno',Keterangan='$cket', no_transaksi='$no_trans' WHERE no_bukti='$cno' AND kd_skpd='$cskpd' AND jenis='$cjenis'");
                echo '1';
            }
        } else if ($jumlah12 > 0) {
            echo '0';
    ...
    public function hapus_data()
    {
        ...
        $no_trans = $this->input->post('no_trans');
        $cjenis = $this->input->post('cjenis');
        $hasil12 = $this->db->query("SELECT count(*) as jumlah FROM jkn_trdtransout WHERE kd_skpd='$skpd' AND no_sp2d='$no_trans'");
        foreach ($hasil12->result_array() as $row) {
            $jumlah12 = $row['jumlah'];
        }
        if ($jumlah12 == 0) {
            $this->db->query("DELETE a from jkn_trdrka a INNER JOIN jkn_trhrka b ON b.kd_skpd=a.kd_skpd AND b.no_bukti=a.no_bukti AND a.jenis=b.jenis WHERE a.kd_skpd='$skpd' AND a.no_bukti='$nomor' AND a.jenis='$cjenis' AND no_transaksi='$no_trans'");
            $this->db->query("DELETE from jkn_trhrka WHERE kd_skpd='$skpd' AND no_bukti='$nomor' AND jenis='$cjenis' AND no_transaksi='$no_trans' AND kd_skpd='$skpd'");
            echo '0';
        } else if ($jumlah12 > 0) {
            echo '1';
        }
