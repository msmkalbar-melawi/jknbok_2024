...
class PenerimaanJKNController extends CI_Controller
{
    ...
    {
        $data['page_title'] = 'INPUT PENERIMAAN JKN';
        $this->template->set('title', 'INPUT PENERIMAAN JKN');
        $this->template->load('template', 'jkn/penerimaan/index', $data);
    }
    ...
    public function cek_simpan()
    {
        ...
        $hasil1 = $this->db->query("SELECT count(*) as jumlah FROM jkn_tr_terima where no_terima='$nomor' and kd_skpd = '$kd_skpd' AND kunci='1'");
    ...
    public function simpan_data()
    {
        ...
        //----------------------------------------------------------------------------------------------------------------------------
        $data = $this->db->insert('jkn_tr_terima', array(
            'no_terima' => $this->input->post('no_terima'),
        ...
        $sql2 = "UPDATE a SET a.status='1' FROM jkn_tr_tetap a WHERE a.no_tetap='$no_terima' and a.kd_skpd='$kd_skpd'";
    ...
    public function loaddata(Type $var = null)
    {
        ...
        $query1->free_result();
        $sql = "SELECT top $rows a.*, (SELECT b.nm_rek6 FROM ms_rek6 b WHERE a.kd_rek6=b.kd_rek6) as nm_rek6, a.sumber FROM jkn_tr_terima a 
                LEFT JOIN jkn_tr_tetap b on a.no_tetap=b.no_tetap and a.kd_skpd=b.kd_skpd
                WHERE a.kd_skpd='$skpd'
                $where AND a.no_terima NOT IN (SELECT TOP $offset a.no_terima FROM jkn_tr_terima a WHERE a.kd_skpd='$skpd' $where 
    ...
    public function load_no_tetap()
    {
        ...
        $sql = "SELECT no_tetap, jenis,tgl_tetap, kd_skpd, keterangan, nilai, kd_rek6, kd_rek_lo, kd_sub_kegiatan,
                (SELECT a.nm_rek6 FROM ms_rek6 a WHERE a.kd_rek6=jkn_tr_tetap.kd_rek6) as nm_rek FROM jkn_tr_tetap $where 
                AND no_tetap not in(select isnull(no_tetap,'') from jkn_tr_terima)
                UNION ALL
                ...
                FROM 
                (SELECT *,(SELECT a.nm_rek6 FROM ms_rek6 a WHERE a.kd_rek6=jkn_tr_tetap.kd_rek6) as nm_rek FROM jkn_tr_tetap $where )a
                LEFT JOIN
                (SELECT no_tetap as tetap,ISNULL(SUM(nilai),0) as nilai_terima from jkn_tr_terima $where GROUP BY no_tetap)b
    ...
    public function delete_data()
    {
        ...
        $sql1 = "DELETE from jkn_tr_terima where no_terima='$nomor' AND kd_skpd='$skpd'";
        $sql2 = "UPDATE a SET a.status='0' FROM jkn_tr_tetap a WHERE a.no_tetap='$nomor' and a.kd_skpd='$skpd'";
        $asg2 = $this->db->query($sql2);
    ...
    {
        $data['page_title'] = 'BKU PENERIMAAN JKN';
        $this->template->set('title', 'BKU PENERIMAAN JKN');
        $this->template->load('template', 'jkn/penerimaan/laporanindex', $data);
    }
    ...
    public function laporanpenerimaanjkn($dcetak = '', $dcetak2 = '', $skpd = '', $jns = '', $spasi = '')
    {
        ...
        $sql1 = "SELECT b.tgl_tetap,b.no_tetap,b.kd_rek6 as kd_rek6penetapan,b.nilai as nil_tetap, b.keterangan as ket_tetap, a.tgl_terima,a.no_terima,a.kd_skpd,a.kd_sub_kegiatan,a.kd_rek6 as kd_rek6penerimaan ,a.nm_rek6, a.nilai as nil_terima,a.keterangan as ket_terima FROM(SELECT kd_skpd, kd_sub_kegiatan, kd_rek6,(SELECT nm_rek6 FROM ms_rek6 WHERE kd_rek6=jkn_tr_terima.kd_rek6) as nm_rek6, no_terima, no_tetap, tgl_terima, nilai,keterangan FROM jkn_tr_terima)a LEFT JOIN (SELECT kd_skpd, kd_sub_kegiatan, kd_rek6, no_tetap,tgl_tetap, nilai, keterangan FROM jkn_tr_tetap) b ON b.kd_skpd=a.kd_skpd AND b.kd_sub_kegiatan=a.kd_sub_kegiatan AND b.no_tetap=a.no_tetap WHERE a.tgl_terima BETWEEN '$dcetak' AND '$dcetak2' AND a.kd_skpd='$skpd' ORDER BY b.tgl_tetap, a.tgl_terima";
    ...
    public function update_data()
    {
        ...
        $usernama      = $this->session->userdata('pcNama');
        $this->db->trans_start();
        $sql            = "DELETE from jkn_tr_terima where kd_skpd='$kd_skpd' AND no_terima='$no_hide'";
        $this->db->query($sql);
        //----------------------------------------------------------------------------------------------------------------------------
        $data = $this->db->insert('jkn_tr_terima', array(
            'no_terima' => $this->input->post('no_terima'),
