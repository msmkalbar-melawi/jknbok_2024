...
class SP2BController extends CI_Controller
{
    ...
    {
        $data['page_title'] = 'SP3B JKN';
        $this->template->set('title', 'SP3B JKN');
        $this->template->load('template', 'jkn/sp2b/index', $data);
    }
    ...
    public function cek_data()
    {
        ...
        $data = $this->db->query("SELECT [status] as status FROM jkn_trhlpj WHERE no_lpj='$no_lpj' AND kd_skpd='$kode'");
        $hasil = $data->row();
        $status = $hasil->status;
        if ($status == '1') {
            echo '1';
    ...
    public function load_data_lpj()
    {
        ...
        FROM
        jkn_trdtransout a
            INNER JOIN jkn_trhtransout b ON a.no_bukti= b.no_bukti 
            AND a.kd_skpd = b.kd_skpd 
        WHERE
            ( a.no_bukti + a.kd_sub_kegiatan + a.kd_rek6 + a.kd_skpd ) NOT IN ( SELECT ( no_bukti + kd_sub_kegiatan + kd_rek6 + kd_skpd ) FROM jkn_trlpj ) 
    ...
    public function simpan_data()
    {
        ...
        $cek = $this->db->query("SELECT no_lpj FROM jkn_trhlpj WHERE kd_skpd='$skpd' AND no_lpj='$nolpj'");
        $hasil = $cek->row();
        if (!empty($hasil)) {
            echo '2';
            return;
    ...
    function load_data()
    {
        ...
        $sql = "SELECT count(*) as tot from jkn_trhlpj WHERE  kd_skpd = '$kd_skpd' AND jenis = '1' $where ";
        $query1 = $this->db->query($sql);
        $total = $query1->row();

        $sql = "SELECT TOP $rows *,(SELECT a.nm_skpd FROM ms_skpd_jkn a where a.kd_skpd = '$kd_skpd') as nm_skpd FROM jkn_trhlpj WHERE kd_skpd = '$kd_skpd' AND jenis = '1' $where 
                AND no_lpj NOT IN (SELECT TOP $offset no_lpj FROM jkn_trhlpj WHERE kd_skpd = '$kd_skpd' AND jenis = '1' $where ORDER BY tgl_lpj,no_lpj) ORDER BY tgl_lpj,no_lpj";
    ...
    public function select_lpj()
    {
        ...
        $sql = "SELECT (select d.tgl_bukti from jkn_trhtransout d left join jkn_trdtransout c on c.no_bukti=d.no_bukti and c.kd_skpd=d.kd_skpd where c.no_bukti=a.no_bukti and c.kd_skpd=a.kd_bp_skpd and c.kd_sub_kegiatan=a.kd_sub_kegiatan and c.kd_rek6=a.kd_rek6) as tgl_bukti,
         a.kd_skpd, a.no_lpj,a.no_bukti,a.kd_sub_kegiatan,a.kd_rek6,a.nm_rek6,a.nilai,kd_bp_skpd FROM jkn_trlpj a INNER JOIN jkn_trhlpj b ON a.no_lpj=b.no_lpj AND a.kd_skpd=b.kd_skpd
    ...
    public function update_data()
    {
        ...
        $cek = $this->db->query("SELECT no_lpj FROM jkn_trhlpj WHERE kd_skpd='$skpd' AND no_lpj='$no_simpan' AND [status]='1'");
        $hasil = $cek->row();
        if (!empty($hasil)) {
            echo '2';
            return;
        }
        $sql1 = $this->db->query("DELETE FROM jkn_trlpj WHERE kd_skpd='$skpd' AND no_lpj='$no_simpan'");
        $sql2 = $this->db->query("DELETE FROM jkn_trhlpj WHERE kd_skpd='$skpd' AND no_lpj='$no_simpan'");
        $sql = "insert into jkn_trlpj $kolom $nilai";
        $asg = $this->db->query($sql);
        if ($asg) {
            $this->db->insert('jkn_trhlpj', array(
    ...
    public function hapus_data()
    {
        ...
        $cek = $this->db->query("SELECT [status] from jkn_trhlpj where no_lpj='$nomolpj' AND kd_skpd='$kd_skpd'")->row();
        $cek1 = $cek->status;
        if ($cek1 == "1") {
            $msg = array('pesan' => '2');
            ...
            $this->db->query("DELETE from jkn_trlpj where no_lpj='$nomolpj' AND kd_skpd='$kd_skpd'");
            $this->db->query("DELETE from jkn_trhlpj where no_lpj='$nomolpj' AND kd_skpd='$kd_skpd'");
            $msg = array('pesan' => '1');
            echo json_encode($msg);
            return;
        }
    ...
    function load_ttd_dinkes($ttd)
    {
        $kd_skpd = substr($this->session->userdata('kdskpd'), 0, 17) . '.0000';
        $sql = "SELECT * FROM ms_ttd WHERE kd_skpd= '1.02.0.00.0.00.01.0000' and kode='$ttd'";
    ...
    public function laporanlpjjkn()
    {
        ...
        $datalpj = $this->db->query("SELECT * FROM jkn_trhlpj WHERE kd_skpd='$lcskpd' AND no_lpj='$no_lpj'")->row();
        if ($jeniscetakan == '0') {
            $judul = 'Surat Permintaan Pengesahan Pendapatan dan Belanja (SP3B)';
            $judul1 = $datalpj->no_sp3b;
        } else if ($jeniscetakan == '1') {
            $judul = 'Laporan Pertanggungjawaban (LPJ)';
            $judul1 = $datalpj->no_lpj;
        }

        // $nilaisebelumnya = $this->db->query("SELECT SUM(c.nil_pend) as penerimaan, SUM(c.nil_peng) as pengeluaran,c.kd_skpd FROM(
        //     SELECT x.kd_skpd, (CASE x.jenis WHEN '1' THEN SUM(x.nilai) ELSE 0 END) AS nil_pend,(CASE x.jenis WHEN '2' THEN SUM(x.nilai) ELSE 0 END) AS nil_peng FROM (
        ...
        //     UNION ALL
        //     SELECT '2' as jenis, ISNULL(SUM(a.nilai),0) as nilai, b.tgl_bukti as tanggal, b.kd_skpd FROM jkn_trdtransout a INNER JOIN jkn_trhtransout b ON b.no_bukti=a.no_bukti AND b.kd_skpd=a.kd_skpd AND b.no_sp2d=a.no_sp2d  WHERE b.tgl_bukti<'$datalpj->tgl_awal' GROUP BY b.tgl_bukti,b.kd_skpd)x GROUP BY x.jenis,x.kd_skpd) c WHERE c.kd_skpd='$lcskpd' GROUP BY c.kd_skpd ");
        ...
        // $this->db->query("recall_skpd_jkn '$lcskpd'");
        ...
        <TR>
        <TD align="center" style="border-collapse:collapse; border-top:solid 1px black; border-left:solid 1px black; border-right:solid 1px black; border-bottom:solid 1px black;" width="13%">
    ...
    FROM
	jkn_trlpj a
	JOIN jkn_trhlpj b ON a.no_lpj = b.no_lpj 
    WHERE
	a.kd_skpd = '$lcskpd' AND b.no_lpj = '$no_lpj' 
    ...
    ";
        ...
        FROM
            ...
            foreach ($hasil->result_array() as $row) {
                ...
                $cRetpendapatan[$i] = "<td width='8%' align='center' style=\"border-collapse:collapse; border-top:solid 1px black; border-left:solid 1px black; border-right:solid 1px black; border-bottom:solid 1px black;\" ><font face=\"Calibri\">" . ($kode1) . "</font></td>
            <td width='18%' align='left' style=\"border-collapse:collapse; border-top:solid 1px black; border-left:solid 1px black; border-right:solid 1px black; border-bottom:solid 1px black;\" ><font face=\"Calibri\">" . ($nama1) . " - " . $nm_rek . "</font></td>
            <td width='22,2%' align='center' style=\"border-collapse:collapse; border-top:solid 1px black; border-left:solid 1px black; border-right:solid 1px black; border-bottom:solid 1px black;\" ><font face=\"Calibri\">Rp&nbsp;" . number_format(($pendapatan), 2, ",", ".") . "</font></td>";
            }
            ...
            foreach ($hasill->result_array() as $row) {
                ...
                if (isset($cRetpendapatan[$j])) {
                    if ($i == $j) {
                        $cRet .= "<tr>
                            " . $cRetpendapatan[$i] . "
                            <td width='12%' align='center' style=\"border-collapse:collapse; border-top:solid 1px black; border-left:solid 1px black; border-right:solid 1px black; border-bottom:solid 1px black;\"><font face=\"Calibri\">" . ($kode2) . "</font></td>
                            <td width='18%' align='left' style=\"border-collapse:collapse; border-top:solid 1px black; border-left:solid 1px black; border-right:solid 1px black; border-bottom:solid 1px black;\"><font face=\"Calibri\">" . ($nama2) . " - " . $nm_rek2 . "</font></td>
                            <td width='22,2%' align='center' style=\"border-collapse:collapse; border-top:solid 1px black; border-left:solid 1px black; border-right:solid 1px black; border-bottom:solid 1px black;\"><font face=\"Calibri\">Rp&nbsp;" . number_format(($belanja), 2, ",", ".") . "</font></td>
                            </tr>";
                    } else {
                        $cRet .= "<tr>
                            ...
                            <td width='22,2%' align='center' ><font face=\"Calibri\"></font></td>
                            <td width='22,2%' align='center' ><font face=\"Calibri\"></font></td>
                            <td width='12%' align='center' ><font face=\"Calibri\">" . ($kode2) . "</font></td>
                            <td width='18%' align='left' ><font face=\"Calibri\">" . ($nama2) . " - " . $nm_rek2 . "</font></td>
                            <td width='22,2%' align='center' ><font face=\"Calibri\">Rp&nbsp;" . number_format(($belanja), 2, ",", ".") . "</font></td>
                            </tr>";
                    }
                } else {
                    $cRet .= "<tr>
            ...
            if ($j == 0 && $i != 0) {
                for ($k = 1; $k <= $i;$k++) {
                    $cRet .= "<tr>
                " . $cRetpendapatan[$k] . "
                <td align=\"center\" style=\"border-collapse:collapse; border-top:solid 1px black; border-left:solid 1px black; border-right:solid 1px black; border-bottom:solid 1px black;\" ><b><font face=\"Calibri\"></font></b></td>
                <td align=\"center\" style=\"border-collapse:collapse; border-top:solid 1px black; border-left:solid 1px black; border-right:solid 1px black; border-bottom:solid 1px black;\" ><b><font face=\"Calibri\"></font></b></td>
                <td align=\"center\" style=\"border-collapse:collapse; border-top:solid 1px black; border-left:solid 1px black; border-right:solid 1px black; border-bottom:solid 1px black;\"><b><font face=\"Calibri\"></font></b></td>
                </tr>";
                }
